...
async def kang_(message: Message):
    ...
    if photo:
        ...
        packname = f"ID_{= f"ID{user.id}_By_DamienRobot_{pack}"
        custom_packnick = Config.CUSTOM_PACK_NAME or f"{u_name}'s DamienX Pack"
        ...
        if ("  A <strong>Telegram</strong> user has created "
            ...
            async with userge.conversation('Stickers', limit=30) as conv:
                ...
                while limit in msg.text:
                    pack += 1
                    packname = f"ID{user.id}_By_DamienRobot_{pack}"
                    packnick = f"{custom_packnick} Vol.{pack}"
                    if is_anim:
                        packname += "_animated"
                        packnick += " (Animated)"
                    ...
                    if msg.text == "Invalid pack selected.":
                        ...
                        if '-d' in message.flags:
                            ...
                            out = "__kanged__" if '-s' in message.flags else \
                                f"**✅ Sticker** [Done](t.me/addstickers/{packname})"
