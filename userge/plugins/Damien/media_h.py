...
from userge.utils import time_formatter, humanbytes, mention_html
...
async def media_h(message: Message):
    ...
    start = time.time()
    await message.edit('This process takes soo much F*ing Time 😂 so here\'s a quote 🙆‍♀️\n\n`\"All you gotta do is chill out... Let go of control and chill out... Let it be, Trust.\"`\n- **Esther Hicks**')
    x = PrettyTable()
    media_dict = {}
    # Generate json
    for m in TYPES:
    ...
    async for msg in userge.search_messages(message.chat.id, "", from_user=reply.from_user.id):
        msg_count += 1
        for media in TYPES:
            if msg[media]:
    ...
    for mediax in TYPES:
        x.add_row([mediax, media_dict[mediax]['count'], humanbytes(media_dict[mediax]['file_size'])])
